...
export function evaluateAgentInvestigation(
  ...
  const metrics = {
    ...
    specialistCoverage: !options.multiAgentExpected
      || ['triage', 'repository', 'action'].every((agent) => reports.some((report) => report.agent === agent)),
    repositoryGrounded: !options.ragExpected
      || (investigation.toolsUsed.includes('search_repository_context') && (investigation.sources?.length ?? 0) > 0),
    safeHighRiskPolicy: actionReport?.risk !== 'high' || investigation.recommendedAction === 'human_review',
    confidenceCalibrated: investigation.confidence >= 0 && investigation.confidence <= 1
