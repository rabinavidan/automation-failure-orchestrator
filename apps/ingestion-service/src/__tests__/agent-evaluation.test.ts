...
const groundedInvestigation: AgentInvestigation = {
  ...
  specialistReports: [
    {
      agent: 'triage',
      summary: 'New critical regression',
      findings: ['401 response'],
      confidence: 0.9,
    },
    {
      agent: 'repository',
      summary: 'Middleware is relevant',
      findings: ['timing-safe comparison'],
      confidence: 0.8,
    },
    {
      agent: 'action',
      summary: 'Review required',
      findings: ['Security path'],
      confidence: 0.9,
...
describe('deterministic agent quality evaluators', () => {
  it('passes grounded, policy-safe supervised investigations', () => {
    const result = evaluateAgentInvestigation(groundedInvestigation, {
      ragExpected: true,
      multiAgentExpected: true,
    });
  ...
  it('fails ungrounded high-risk automation and reports actionable metrics', () => {
    const unsafe: AgentInvestigation = {
      ...
      toolsUsed: [],
      sources: [],
      specialistReports: groundedInvestigation.specialistReports?.filter(
        (report) => report.agent !== 'repository'
      ),
    };
    const result = evaluateAgentInvestigation(unsafe, {
      ragExpected: true,
      multiAgentExpected: true,
    });
